...
#nullable disable
  internal class ByteBankAtendimento
  {
    ...
    public void AtendimentoCliente()
    {
      try
      {
        ...
        while (opcao != '8')
        {
          ...
          Console.WriteLine("===6 - Exportar em JSON     ===");
    ...
    private void ExportarContas()
    {
      ...
      {
        //O NewtonSoft.Json é um pacote muito utilizado no universo .NET que
        //permite que façamos a serialização e desserialização de objetos para o formato JSON de maneira bem simples.
        //https://www.newtonsoft.com/json
        string json = JsonConvert.SerializeObject(_listaDeContas, Formatting.Indented);
        try
        {
          using (var fs = new FileStream(@"C:\Users\Biel\Documents\workspaces\vs\ByteBank_Atendimento\tmp\export\contas.json", FileMode.Create))
    ...
    private void ExportarContasEmXML()
    {
      ...
      else
      {
        ...
        try
        {
          using (var fs = new FileStream(@"C:\Users\Biel\Documents\workspaces\vs\ByteBank_Atendimento\tmp\export\contas.xml", FileMode.Create))
          using (var streamwriter = new StreamWriter(fs))
          {
